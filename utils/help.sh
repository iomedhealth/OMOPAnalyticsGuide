...
help() {
    ...
    echo -e "Usage:"
    echo -e "  make ${target_col}[target]${col_off} ${variable_col}[variables]${col_off}\\n"
...
_help_targets() {
    ...
    grep -E "$pattern" "$1" | sort | while read -r line; do
        target=${line%%:*}
        description=${line#*## }
        echo -e "  ${target_col}${target}${col_off} ${description}"
    done
    echo ""
}
...
_help_variables() {
    ...
    grep -E "$pattern" "$1" | sort | while read -r line; do
        ...
        default=${default%%##*}
        description=${line##*## }
        echo -e "  ${variable_col}${variable}${col_off} ${description} ${grey}(default: ${default})${col_off}"
    done
    echo ""
}
